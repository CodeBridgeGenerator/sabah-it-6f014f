...
import personalFakerFactory from "./personalFakerFactory";
...
const PersonalFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = personalFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("personal").create(elem));
...
export default connect(mapState, mapDispatch)(PersonalFakerDialogComponent);
