...
const MaintenanceRequestsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.maintenancereport}</p>
...
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.priority}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "maintenanceRequests"});
...
<Column field="buildingid" header="Buildingid" body={pTemplate0} filter={selectedFilterFields.includes("buildingid")} hidden={selectedHideFields?.includes("buildingid")}  sortable style={{ minWidth: "8rem" }} />
<Column field="maintenancereport" header="Maintenancereport" body={pTemplate1} filter={selectedFilterFields.includes("maintenancereport")} hidden={selectedHideFields?.includes("maintenancereport")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="priority" header="Priority" body={pTemplate3} filter={selectedFilterFields.includes("priority")} hidden={selectedHideFields?.includes("priority")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload MaintenanceRequests Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search MaintenanceRequests" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
